...
public class HuffProcessor {
	...
	public void compress(BitInputStream in, BitOutputStream out){
		...
		writeCompressed(in, out, encodings);
	...
	private void makeEncodings(HuffNode root, String path, String[] encodings){
		...
		if(root.left == null && root.right == null){
			...
			//Note: in APT, I make the root null after this
			return;
		}
	...
	private void writeTree(HuffNode root, BitOutputStream out){
		...
		//Would really rather make this second, not first - let's see in debugging
		if(!(root.left == null && root.right == null)){
			out.writeBits(1, 0);
	...
	private void writeCompressed(BitInputStream in, BitOutputStream out, String[] encodings){
		in.reset();
		while(true){
			int val = in.readBits(BITS_PER_WORD);
			if(val == -1){
			...
			//int thisChar = Integer.parseInt(curr, 2);
	...
	public void decompress(BitInputStream in, BitOutputStream out){
		...
		while(true){
			int currBit = in.readBits(1);
			if(currBit == -1){
				throw new HuffException("bad input, no PseudoEOF");
			}
			else{
				if(currBit == 0){
